...
import { buildShop, SHOP_LEVELS, applyPurchasedEffects, type LockedItem, type CharacterState } from './_lib/shop'
import { type ClassName, STAT_META } from './_lib/character'
import { CharacterEffectsDisplay } from './_components/character-effects-display'
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="p-3 sm:p-6 mt-3 sm:mt-6 animate-pulse flex items-center justify-center gap-2">
        ...
        <h1 className="font-old text-3xl sm:text-5xl text-red-600 flex items-center justify-center">One Shot Dungeon</h1>
        <FlameKindling className="size-8 sm:size-12 text-amber-500" />
      </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-96 max-w-xl bg-zinc-900/50 backdrop-blur-md rounded-lg mb-3 sm:mb-6 w-full">
        {match(mode)
          ...
          .with({ mode: 'character' }, () => <CharacterSelect onSelect={(character) => setMode({ mode: 'summary', character })} />)
          ...
          .with({ mode: 'shop' }, (state) => {
            ...
            const carriedLockedRemaining = Object.fromEntries(
              builtShop.carriedLocks.map((entry) => [entry.item.instanceId, entry.remainingAppearances]),
            )
            ...
            return (
              <ShopScreen
                ...
                onFinish={({ coins: nextCoins, remainingLocks, purchasedItems, nextLockedItems, extraLifePurchasesDelta }) => {
                  setCoins(nextCoins)
                  setExtraLifePurchases((count) => count + extraLifePurchasesDelta)
                  setLockCharges(remainingLocks)
                  setLockedItems(nextLockedItems)

                  const { nextCharacter, gainedExtraLives } = applyPurchasedEffects(state.character, purchasedItems)
          ...
          .with({ mode: 'game' }, (state) => (
            <div className="flex flex-col items-center justify-center p-3 gap-3 animate-in fade-in duration-500">
              <h2 className="font-old text-2xl text-amber-500">Welcome to the Dungeon, {state.character.name}.</h2>
              <div className="flex gap-2 font-old text-zinc-300 bg-zinc-950/80 p-3 border border-zinc-800">
                {STAT_META.map(({ label, key, color }) => (
                  <div key={label}>
                    <span className="text-zinc-500 text-sm block">{label}</span>
                    <span className={`text-2xl ${color}`}>{state.character.stats[key]}</span>
                  </div>
                ))}
              </div>
              {state.character.effects && (
                <div className="flex gap-2 font-old text-amber-500 bg-amber-950/50 p-2 border border-amber-900 w-full justify-center">
                  <CharacterEffectsDisplay effects={state.character.effects} />
                </div>
              )}
              ...
              <p className="text-zinc-400 text-center">Next shops are scheduled after levels {SHOP_LEVELS.join(', ')}.</p>
